...
fn main() {
    ...
    let user_settings = match get_user_settings(config_path) {
        Err(err) => match err {
            ...
            custom_errors::SettingsError::Io(err) => {
    ...
    while !rl.window_should_close() {
        ...
        if shift_key_down {
            let min_circle_size = render_size_avg * 0.02;
            let max_circle_size = render_size_avg / 3.0;
            ...
            circle_size = circle_size.clamp(min_circle_size, max_circle_size);
        }
    }
}
